...
def parse_args(argv):
    ...
    p.add_argument("file",nargs="+",help="Convert whl file")
    p.add_argument("--depends", nargs="+", help="")
    p.add_argument("--name", default="", help="")
    p.add_argument(
        "-o",
        "--output",
        default="output",
...
def debianize(args):
    ...
    for wheel in wheels:
        ...
        ctx = settings.get_ctx(wheel.filename)
        package = SourcePackage(ctx, wheel, args.output, extras=wheels, name=args.name)
        package.create()
